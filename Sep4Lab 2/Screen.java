...
public class Screen extends JPanel implements ActionListener, KeyListener, MouseListener, MouseMotionListener{
    ...
    private JButton sortButton, randomizeButton, addSongButton, removeByIndexButton;
    ...
    public void actionPerformed(ActionEvent e){
        if (e.getSource() == addSongButton) {
            ...
            for (int i = playlist.size() - 1; i > 0; i--) {
                for (int j = 0; j < i; j++) {
                    if (playlist.get(j).toString().toLowerCase().compareTo(playlist.get(j+1).toString().toLowerCase()) > 0) {
